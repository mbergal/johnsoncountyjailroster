...
namespace JailRoster
    {
    internal class ScrapeCommand : ConsoleCommand
        {
        ...
        static private Detainee   ScrapeDetainee( string url )
            ...
            var detainee = new Detainee()
                {
                    JailRosterId = Int32.Parse( doc.DocumentNode.SelectSingleNode( "//input[@id='jID']").GetAttributeValue( "value", "" ) ),
                    Name = doc.DocumentNode.SelectSingleNode( "//input[@id='n']").GetAttributeValue( "value", "" ),
                    Age = Int32.Parse( doc.DocumentNode.SelectSingleNode( ".//table[@class='data']/tr/td").InnerText ),
